...
// --- AUDIO SYSTEM ----------------------------------------------------
// Use ONE persistent HTMLAudioElement for the entire app.
// This is much more reliable on iOS/Safari than repeatedly creating
// new Audio() objects for every lesson/question/card.
...
let currentAudio = audioPlayer; // kept for compatibility with existing code
...
function stopLessonAudio() {
  // Invalidate any pending play/load operation.
  audioRequestId++;
...
function waitForAudioReady(requestId) {
  if (requestId !== audioRequestId) {
    return Promise.reject(new Error("Audio request superseded"));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const onReady = () => {
      if (requestId !== audioRequestId) {
        finish(() => reject(new Error("Audio request superseded")));
        return;
      }
    ...
    const onError = () => {
      finish(() => reject(
        audioPlayer.error || new Error("Audio loading failed")
      ));
    };
    ...
    const timeoutId = setTimeout(() => {
      if (requestId !== audioRequestId) {
        ...
        audioPlayer.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA
      ) {
        finish(resolve);
      } else {
        finish(() => reject(new Error("Audio loading timed out")));
      }
    }, 10000);
...
async function playAudio(path) {
  ...
  try {
    // Always stop whatever was playing first.
    audioPlayer.pause();
    ...
    await waitForAudioReady(requestId);

    // Another audio request may have happened while this file was loading.
    if (requestId !== audioRequestId) {
      return false;
    }
    ...
    // Ignore intentionally cancelled/superseded requests.
    if (requestId !== audioRequestId) {
      return false;
    }
...
async function unlockAudio() {
  ...
  audioUnlockPromise = (async () => {
    try {
      // Use the existing home audio as the unlock audio.
      // This keeps the original behavior: the first user interaction
      // starts the home song.
      const unlockPath = "audio/cover_song64kbps.mp3";

      audioRequestId++;
      currentAudioPath = unlockPath;
      audioPlayer.src = unlockPath;
      audioPlayer.load();

      // IMPORTANT: this function is called directly from a real
      // user interaction, so this play() occurs inside the gesture.
...
const CHAPTER_STARTS = [
  0, 4, 5, 9, 13, 17, 18, 22, 26, 30,
  31, 34, 37, 41, 42, 46, 51, 56, 63, 67,
  68, 69, 72, 76, 77, 80, 83, 89, 93, 94,
  98, 103, 108, 113, 117, 121, 122, 125, 130,
  134, 135, 136, 137, 138, 139, 143, 144, 147
];
...
function buildMainLessons() {
  const subRanges = SUBLESSONS.map(
    ([start, count]) => ({
      start,
      end: start + count - 1
    })
...
function getRepeatGroup(lessonIdx) {
  if (NO_REPEAT_LESSONS.has(lessonIdx)) {
    return null;
  }
...
// Mobile browsers require audio playback to originate from a user
// gesture. Unlock the persistent audio element on the first interaction.
...
// Also listen on document because some mobile interactions may land on
// an element outside the Rive canvas.
firstInteractionEvents.forEach(event => {
...
async function loadLessonTexts() {
  ...
  rows.forEach(row => {
    ...
    [
      ...
      if (!lessonTexts[t][langIdx]) {
        lessonTexts[t][langIdx] = {};
      }

      lessonTexts[t][langIdx][`line${l}`] =
        (val ?? "").trim();
    });
  });
}
...
function goToLesson(idx) {
  ...
  const langIdx =
    Math.round(
      vm.number("languageIdx").value
    );

  const texts =
    lessonTexts[currentLessonIdx]?.[langIdx];
  ...
  const data =
    LESSON_DATA[currentLessonIdx];

  if (!data) return;

  lessonVM.number("lessonIdx").value =
    currentLessonIdx;

  lessonVM.number("lessonType").value =
    data.type;

  for (let i = 1; i <= 7; i++) {
    lessonVM.number(`pos${i}`).value =
      data[`pos${i}`];

    lessonVM.number(`vocab${i}`).value =
      data[`vocab${i}`];
  }
}
...
function setVocabTexts(langIdx) {
  ...
  const vocabVM =
    vm.viewModel("propertyOfVocabularyVM");

  const startIdx =
    vocabPage * CARD_COUNT;
...
function setCurrentQuestion(idx) {
  ...
  const slotIdx =
    imageList.indexOf(vocabIdx);

  currentVocabIdx =
    vocabIdx;

  gameVM.number("current").value =
    slotIdx;

  // Start the question audio through the persistent audio player.
  // Avoid setTimeout here because mobile browsers can treat delayed
  // playback differently from playback initiated by the interaction.
  const audioFile =
    vocabTexts[vocabIdx]?.[3];
...
function poll() {
  ...
  if (vm) {
    ...
    if (stateNum !== lastStateNum) {
      ...
      // Stop the persistent player whenever the app changes state.
      stopLessonAudio();
...
const r = new rive.Rive({
  ...
  onLoad: async () => {
        ...
        if (lastState !== 1) {
          ...
          switch (stateNum) {
            ...
            case 3:
              ...
              vm.viewModel(
                "propertyOfVocabControlVM"
              ).trigger(
                "returnTriggerVocabPage"
              ).trigger();
            ...
            case 4:
              ...
              vm.viewModel(
                "propertyOfPatternControlVM"
              ).trigger(
                "patternReturnTrigger"
              ).trigger();
    ...
    const mediaPlayerVM =
      vm.viewModel(
        "propertyOfMediaPlayerVM"
      );


    mediaPlayerVM.trigger(
      "forwardTrigger"
    ).on(() => {
    ...
    gameControlVM.trigger(
      ...
      if (listenAgainDebounce) {
        return;
      }

      if (currentVocabIdx === -1) {
        return;
      }
    ...
    // vocab page triggers
    const vocabControlVM =
      vm.viewModel(
        "propertyOfVocabControlVM"
      );


    vocabControlVM.trigger(
      "nextPageTrigger"
    ).on(() => {
    ...
    vocabVM.trigger(
      ...
      // get audio filename from vocabTexts[vocabIdx][3]
      ...
      // Stop any playing audio and play the new audio
      // through the persistent player.
      // This prevents overlapping/racing Audio objects.
      playAudio(
        `audio/${audioFile}.mp3`
      );
    ...
    const patternControlVM =
      vm.viewModel(
        "propertyOfPatternControlVM"
      );


    patternControlVM.trigger(
      "patternNextTrigger"
    ).on(() => {
    ...
    const AlphabetVM =
      vm.viewModel(
        "propertyOfAlphabetVM"
      );


    const AlphabetControlVM =
      vm.viewModel(
        "propertyOfAlphabetControlVM"
      );


    AlphabetControlVM.trigger(
      "nextalpatrigger"
    ).on(() => {
