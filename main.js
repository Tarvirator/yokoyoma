...
function waitForAudioReady(requestId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onError = () => {
            finish(() => reject(audioPlayer.error || new Error("Audio loading failed")));
        };
        ...
        const timeoutId = setTimeout(() => {
            ...
            else if (audioPlayer.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA) {
                finish(resolve);
            }
            else {
...
const CHAPTER_STARTS = [
    0, 13, 26, 37, 63, 72, 89, 113, 130, 147
];
...
function buildMainLessons() {
    const subRanges = SUBLESSONS.map(([start, count]) => ({
        start,
        end: start + count - 1
    }));
    const mains = [];
...
function playHomeAudioOnFirstInteraction() {
    ...
    if (vm) {
        ...
        // Homepage is state 0.
        if (stateNum !== 0) {
            console.log("First interaction was outside homepage — skipping cover song.");
            ...
            firstInteractionEvents.forEach(event => {
                ...
                document.removeEventListener(event, playHomeAudioOnFirstInteraction);
            });
            return;
        }
    }
...
firstInteractionEvents.forEach(event => {
    document.addEventListener(event, playHomeAudioOnFirstInteraction, {
        passive: true
    });
});
    ...
    if (vm) {
        ...
        if (stateNum === 1 && !lessonResetting) {
            ...
            if (startEnd === 1 &&
                ...
                if (group) {
                    if (!activeRepeatGroup ||
                        activeRepeatGroup[0] !== group[0]) {
                        activeRepeatGroup = group;
                        repeatPlayCount = 1;
                    }
                    const isLastInGroup = currentLessonIdx ===
    ...
    onLoad: async () => {
            ...
            if (lastState !== 1) {
                ...
                switch (stateNum) {
                    ...
                    case 5:
                        ...
                        break;
                    case 6:
                        vm.viewModel("propertyOfSongcontrolVm1").trigger("songReturn").trigger();
                        break;
                    default:
                        vm.trigger("exittrigger").trigger();
                        break;
        ...
        mediaPlayerVM.trigger("endTrigger").on(() => {
            ...
            if (next <
                ...
                goToLesson(CHAPTER_STARTS[next]);
            }
        });
        ...
        mediaPlayerVM.trigger("startTrigger").on(() => {
            ...
            if (prev >= 0) {
                goToLesson(CHAPTER_STARTS[prev]);
            }
        });
        ...
        patternControlVM.trigger("patternPrevTrigger").on(() => {
            ...
            patternVM.number("patternPageIdx").value = Math.max(0, current - 1);
                ...
                // Alphabet page triggers
        const AlphabetVM = vm.viewModel("propertyOfAlphabetVM");
        const AlphabetControlVM = vm.viewModel("propertyOfAlphabetControlVM");
        const Alphabet1VM = vm.viewModel("propertyOfAlphabet1");

        Alphabet1VM.trigger("alphabetTrigger").on(() => {
            const currentPage = Math.round(
                AlphabetVM.number("alphabetPage").value
            );
            ...
            const alphabetAudio = Alphabet1VM.string("alphabetAudio").value;

            if (!alphabetAudio) {
                return;
            }

            playAudio(`audio/${alphabetAudio}.mp3`);
        });
